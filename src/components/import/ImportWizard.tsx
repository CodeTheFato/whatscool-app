...
import type { ImportWizardConfig, PreviewRow } from "./types"

type Step = 1 | 2 | 3

interface ImportWizardProps {
  config: ImportWizardConfig
  mockPreview: PreviewRow[]
}

export function ImportWizard({ config, mockPreview }: ImportWizardProps) {
  ...
  const validCount = mockPreview.filter((r) => r.status === "valid").length
  const errorCount = mockPreview.filter((r) => r.status === "error").length
  ...
  const handleImport = async () => {
    ...
    toast.success(`${validCount} ${config.entityLabel} importados com sucesso!`)
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="rounded-lg border bg-card p-4 text-center">
              <p className="text-2xl font-semibold">{mockPreview.length}</p>
            ...
            <div className="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-900 dark:bg-amber-950/30">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-amber-700 dark:text-amber-300">
                  As linhas com erro serão ignoradas. Apenas {validCount === 1 ? `1 ${config.entityLabelSingular} válido será importado` : `os ${validCount} ${config.entityLabel} válidos serão importados`}.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {isProcessing ? "Importando..." : `Importar ${validCount} ${config.entityLabel}`}
              </Button>
            </div>
          </div>
        </div>
